...
```{r}
plot_ward_population <- function(tbl, district_name) {
  ...
  # Ward populations and affected wards
  geom_sf(aes(colour = rdna, fill = population_total), linewidth = .7, alpha = .8) +
  # River
  geom_sf(data = river, colour = "#002e6e", fill = "#002e6e", alpha = .6) +
  # Settlements
...
```{r}
wards_joined |> filter(adm2_en == "Nuwakot") |> 
  count(rdna)
```


```{r}
rasuwa_population_map <- plot_ward_population(census_working, "Rasuwa") +
...
rasuwa_poorest_quintile <- census_working |>
  ...
  mutate(ward_label = paste0(ward, "|", wealth_quintile_poorest_quintile_pc, "%")) |> 
  st_as_sf() |> 
  ggplot() +
  geom_sf(data = adm3, alpha = .25, linewidth = .5, colour = "grey70", fill = NA) +
  geom_sf_text(data = adm3 |> filter(adm2_name == "Rasuwa"), 
...
```{r number-agencies-sector-facet, warning=FALSE}
consolidated |> 
  mutate(sector = ifelse(str_detect(sector, "Protection"), "Protection", sector), 
...
consolidated |> 
  ...
  scale_fill_continuous(high = "#004987", low = "#e3edf6", labels = comma, na.value = "grey97", 
                        trans = "log10") +
  facet_wrap(~sector) + 
  labs(title = "Number of Beneficiary frequencies reached, by municipality", 
...
ggsave("./plots/rasuwa_frequencies_facet_map_small.png", height = 5, width = 7, units = "in")

ggsave("./plots/rasuwa_frequencies_facet_map.png", height = 8.27, width = 11.69, units = "in")
```
