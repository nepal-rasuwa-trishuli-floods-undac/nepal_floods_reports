...
The [population](https://censusresults.nsonepal.gov.np/) of the flood-affected wards in Rasuwa district is `r filter(population_projections, district == "Rasuwa") %>% {sum(.$projection_2026_population_total)}  |> format(big.mark = ",")`. Of the `r census_flat |> filter(district == "Rasuwa") |> nrow()` wards in Rasuwa district, `r differences |> filter(district == "Rasuwa") |> nrow()` have been affected by the flooding, according to the NDRRMA. 
